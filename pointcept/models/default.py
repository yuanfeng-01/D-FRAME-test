...
@MODELS.register_module()
class DefaultSegmentorV8(nn.Module):
    def __init__(
        self,
        num_classes,
        backbone_out_channels,
    ...
    def forward(self, input_dict):
        ...
        if isinstance(point, Point):
            ...
            feat = point
        pred_res = input_dict["coord"] + self.pred_head(self.conv(feat))  # [BN, 3]
        # seg_logits = torch.nn.functional.adaptive_max_pool1d(seg_logits.permute(0, 2, 1), M).permute(0, 2, 1)
        # train
        if self.training:
            loss = self.criteria([pred_res, input_dict["sampled_num"], input_dict["sampled_num"]], input_dict["segment"])
            return dict(loss=loss)
        # eval
        elif "segment" in input_dict.keys():
            loss = self.criteria([pred_res, input_dict["sampled_num"], input_dict["sampled_num"]], input_dict["segment"])
...
@MODELS.register_module()
class DefaultSegmentorV9(nn.Module):
    def __init__(
        self,
        num_classes,
        backbone_out_channels,
        ...
        assert len(criteria) == 2
        self.criteria_cls = build_criteria(criteria[0])
        self.criteria_cd = build_criteria(criteria[1])
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            loss_seg = self.criteria_cls(seg_logits, input_dict["segment"])
            loss_cd = self.criteria_cd([pred_res, input_dict["sampled_num"], input_dict["dense_gt_num"], input_dict["segment"]], input_dict["dense_pc"])
            loss = loss_seg + loss_cd
            return dict(loss=loss, loss_seg=loss_seg, loss_cd=loss_cd)
        # eval
        elif "segment" in input_dict.keys():
            loss_seg = self.criteria_cls(seg_logits, input_dict["segment"])
            loss_cd = self.criteria_cd([pred_res, input_dict["sampled_num"], input_dict["dense_gt_num"], input_dict["segment"]], input_dict["dense_pc"])
            loss = loss_seg + loss_cd
            return dict(loss=loss, loss_seg=loss_seg, loss_cd=loss_cd, pred=pred_res, seg_logits=seg_logits)
        # test
        else:
            return dict(pred=pred_res)
