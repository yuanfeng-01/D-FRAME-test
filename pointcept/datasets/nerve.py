...
@DATASETS.register_module()
class NerVEDatasetV6(DefaultDataset):
    def __init__(
        self,
        lr_file=None,
        la_file=None,
...
@DATASETS.register_module()
class NerVEDatasetV7(DefaultDataset):
    def __init__(
        self,
        lr_file=None,
        la_file=None,
    ...
    def get_data(self, idx):
        ...
        if pred is None:
            cls_label = temp_dict["label"].astype(np.int32)
            data_dict["coord"] = ori_pc[cls_label == 1].astype(np.float32)
            data_dict["segment"] = data_dict["coord"].copy()

            # drop
            keep_num = int(0.95 * data_dict["coord"].shape[0])
            crop_num = data_dict["coord"].shape[0] - keep_num
            idx = np.random.choice(data_dict["coord"].shape[0], keep_num, replace=False)
            data_dict["coord"] = data_dict["coord"][idx]

            # interp
            interp_idx = np.random.choice(data_dict["coord"].shape[0], crop_num, replace=False)
            interp_cen = data_dict["coord"][interp_idx]
            _, knn_idx = self.get_knn_dis(interp_cen, ori_pc, k=5)
            interp_pts = np.take(ori_pc, knn_idx, axis=0).mean(axis=1)
            data_dict["coord"] = np.concatenate([data_dict["coord"], interp_pts], axis=0)
            # print(data_dict["coord"].shape, data_dict["segment"].shape)

            # noise
            bbox_min = np.min(ori_pc, axis=0)
            bbox_max = np.max(ori_pc, axis=0)
            diagonal_length = np.linalg.norm(bbox_max - bbox_min)

            sigma = 0.005 * diagonal_length

            # 生成高斯噪声并添加到点云
            noise = np.random.normal(loc=0, scale=sigma, size=data_dict["coord"].shape)
            jitter = np.clip(noise, a_min=-0.01, a_max=0.01)
            # jitter = np.clip(
            #     0.03 * np.random.randn(data_dict["coord"].shape[0], 3), -0.01, 0.01,
            # )
...
@DATASETS.register_module()
class NerVEDatasetV8(DefaultDataset):
    def __init__(
        self,
        lr_file=None,
        la_file=None,
    ...
    def get_data(self, idx):
        ...
        # data_dict["normal"] = data_dict["normal"].astype(np.float32)
        data_dict["segment"] = temp_dict["label"].astype(np.int32)
        data_dict["dense_pc"] = temp_dict["gt_pts"].astype(np.float32)
