...
class RandomJitter(object):
    def __init__(self, sigma=0.01, clip=0.05):
        ...
        self.sigma = sigma
        self.clip = clip
        print('jitter args: ', sigma, clip)

    def __call__(self, data_dict):
        if "coord" in data_dict.keys() and 'segment' in data_dict.keys():
            jitter = np.clip(
                self.sigma * np.random.randn(data_dict["coord"][data_dict["segment"] == 1].shape[0], 3),
                -self.clip,
                self.clip,
            )
            data_dict["coord"][data_dict["segment"] == 1] += jitter
        return data_dict


@TRANSFORMS.register_module()
class RandomJitterWithCopy(object):
    def __init__(self, sigma=0.01, clip=0.05, jitter_application_ratio=0.2, copy_ratio=0.05):
        assert clip > 0
        self.sigma = sigma
        self.clip = clip
        self.jitter_application_ratio = jitter_application_ratio
        self.copy_ratio = copy_ratio

    def __call__(self, data_dict):
        if random.random() < self.jitter_application_ratio:
            n = len(data_dict["coord"])
            idx = np.random.choice(n, int(n * self.copy_ratio), replace=False)
            jitter = np.clip(
                self.sigma * np.random.randn(len(idx), 3),
                -self.clip,
                self.clip,
            )
            if "sampled_index" in data_dict:
                ...
                data_dict["sampled_index"] = np.where(mask[idx])[0]
            if "coord" in data_dict.keys():
                copy_coord = data_dict["coord"][idx] + jitter
                data_dict["coord"] = np.concatenate([data_dict["coord"], copy_coord], axis=0)
            if "color" in data_dict.keys():
                copy_color = data_dict["color"][idx]
                data_dict["color"] = np.concatenate([data_dict["color"], copy_color], axis=0)
            if "normal" in data_dict.keys():
                copy_normal = data_dict["normal"][idx]
                data_dict["normal"] = np.concatenate([data_dict["normal"], copy_normal], axis=0)
            if "strength" in data_dict.keys():
                copy_strength = data_dict["strength"][idx]
                data_dict["strength"] = np.concatenate([data_dict["strength"], copy_strength], axis=0)
            if "segment" in data_dict.keys():
                copy_segment = data_dict["segment"][idx]
                data_dict["segment"] = np.concatenate([data_dict["segment"], copy_segment], axis=0)
            if "instance" in data_dict.keys():
                copy_instance = data_dict["instance"][idx]
                data_dict["instance"] = np.concatenate([data_dict["instance"], copy_instance], axis=0)
        return data_dict
...
class GridSample(object):
    def __init__(
        ...
        return_sampled_num=False,
        return_dense_gt_num=False,
        ...
        self.return_sampled_num = return_sampled_num
        self.return_dense_gt_num = return_dense_gt_num
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "train":  # train mode
            ...
            if self.return_dense_gt_num:
                data_dict["dense_gt_num"] = np.array([data_dict["dense_pc"].shape[0]]).astype(np.int32)
            for key in self.keys:
                data_dict[key] = data_dict[key][idx_unique]
            if self.return_sampled_num:
                data_dict["sampled_num"] = np.array([len(idx_unique)]).astype(np.int32)
            return data_dict
            ...
            for i in range(count.max()):
                ...
                if self.return_sampled_num:
                    data_dict["sampled_num"] = np.array([len(idx_part)]).astype(np.int32)
                if self.return_dense_gt_num:
                    data_dict["dense_gt_num"] = np.array([data_dict["dense_pc"].shape[0]]).astype(np.int32)
