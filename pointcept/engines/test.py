...
class MseTester(TesterBase):
    def test(self):
        ...
        if (
            ...
            or self.cfg.data.test.type == "TeethLandDataset"
        ...
        for idx, data_dict in enumerate(self.test_loader):
            input_dict = data_dict[0]
            data_name = input_dict.pop("name")
            # if data_name != "00000006":
            #     continue
            pred_save_path = os.path.join(save_path, "{}.txt".format(data_name))
            ...
            elif self.cfg.data.test.type == "TeethLandDataset":
                pred = pred.cpu().numpy()
                coord = input_dict["coord"].cpu().numpy()
            elif self.cfg.data.test.type == "NerVEDatasetV2":
                inp = torch.sqrt(torch.sum(pred.mul(pred), dim=-1) + 0.00000001).view(pred.shape[0], 1)
                pred = pred / (inp + 0.0000000001)
                pred = pred.cpu().numpy()
                # pred /= (np.sqrt(np.sum(pred ** 2, axis=-1)) + 1e-8)
                coord = input_dict["coord"].cpu().numpy()
            else:
            ...
            np.savetxt(pred_save_path, np.concatenate([coord, pred], axis=-1))
        ...
        #             )
        #         )
        logger.info("Val result: loss {:.4f}".format(losses / len(self.test_loader)))
...
class EdgeTester(TesterBase):
    def test(self):
        ...
        target_meter = AverageMeter()
        self.model.eval()
        logger.info(self.cfg.data_root)
        ...
        if (
            ...
            or self.cfg.data.test.type == "ScanNet200Dataset"
            or self.cfg.data.test.type == "ScanNetPPDataset"
            or self.cfg.data.test.type == "NerVEDataset"
        ) and comm.is_main_process():
            make_dirs(os.path.join(save_path, "submit"))
        elif (
            self.cfg.data.test.type == "SemanticKITTIDataset" and comm.is_main_process()
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            input_dict = data_dict[0]  # current assume batch size is 1
            # fragment_list = data_dict.pop("fragment_list")
            # segment = data_dict.pop("segment")
            data_name = input_dict.pop("name")
            # if data_name != "00000003":
            #     continue
            ...
            if os.path.isfile(pred_save_path):
                ...
                pred = np.loadtxt(pred_save_path)
                if "origin_segment" in input_dict.keys():
                    segment = input_dict["origin_segment"]
            else:
                # pred = torch.zeros((segment.shape[0], self.cfg.data.num_classes)).cuda()
                for key in input_dict.keys():
                    if isinstance(input_dict[key], torch.Tensor):
                        input_dict[key] = input_dict[key].cuda(non_blocking=True)
                with torch.no_grad():
                ...
                # loss = output_dict["loss"]
                pred = output.max(1)[1]
                # pred = (torch.softmax(output, dim=-1)[:, -1] > 0.1).int()
                ...
                if self.cfg.data.test.type == "ScanNetPPDataset":
                    pred = pred.topk(3, dim=1)[1].data.cpu().numpy()
                elif self.cfg.data.test.type == "NerVEDataset":
                    pred = pred.data.cpu().numpy()
                    segment = segment.data.cpu().numpy()
                    coord = input_dict["coord"].cpu().numpy()
                else:
                    pred = pred.max(1)[1].data.cpu().numpy()
                if "origin_segment" in input_dict.keys():
                    ...
                    pred = pred[input_dict["inverse"]]
                    segment = input_dict["origin_segment"]
                np.savetxt(pred_save_path, np.concatenate([coord, segment[:, None], pred[:, None]], axis=-1))
                # np.savetxt(pred_save_path.replace('pred', 'out'), torch.softmax(output, dim=-1).detach().cpu().numpy())
            batch_time.update(time.time() - end)
            if (
                self.cfg.data.test.type == "ScanNetDataset"
                or self.cfg.data.test.type == "ScanNet200Dataset"
...
class EdgeTesterV2(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                ...
                with torch.no_grad():
                    output_dict = self.model(input_dict)
                output = output_dict["seg_logits"]
                pred_direction = output_dict["pred"]
                pred_loss = output_dict["loss_pred"]
                losses += pred_loss
                pred = output.max(1)[1]
                segment = input_dict["segment"]
                # gt_direction = input_dict["direction"]
                if "origin_coord" in input_dict.keys():
                    idx, _ = pointops.knn_query(
                        1,
                        input_dict["coord"].float(),
                ...
                if self.cfg.data.test.type == "ScanNetPPDataset":
                    pred = pred.topk(3, dim=1)[1].data.cpu().numpy()
                elif self.cfg.data.test.type == "NerVEDatasetV3":
                    pred = pred.data.cpu().numpy()
                    segment = segment.data.cpu().numpy()
                    coord = input_dict["coord"].cpu().numpy()
                    inp = torch.sqrt(torch.sum(pred_direction.mul(pred_direction), dim=-1) + 0.00000001).view(pred_direction.shape[0], 1)
                    pred_direction = pred_direction / (inp + 0.0000000001)
                    pred_direction = pred_direction.data.cpu().numpy()
                    np.savetxt(pred_save_path, np.concatenate([coord, pred_direction, segment[:, None], pred[:, None]], axis=-1))
                else:
                    pred = pred.max(1)[1].data.cpu().numpy()
                if "origin_segment" in input_dict.keys():
                    assert "inverse" in input_dict.keys()
        ...
        if comm.is_main_process():
            ...
            logger.info(
                "Val result: mIoU/mAcc/allAcc/pred_loss {:.4f}/{:.4f}/{:.4f}/{:.4f}".format(
                    mIoU, mAcc, allAcc, losses/num
                )
            )
            for i in range(self.cfg.data.num_classes):
                logger.info(
...
class EdgeTesterV3(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            input_dict = data_dict[0]
            data_name = input_dict.pop("name")
            if data_name != "00000068" and data_name != '00004156':
                continue
            pred_save_path = os.path.join(save_path, "{}_pred.txt".format(data_name))
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
            ...
            batch_time.update(time.time() - end)
            logger.info(
                "Test: {}/{}-{data_name}  Batch {batch_time.val:.3f} ({batch_time.avg:.3f})".format(
                    idx + 1,
                    len(self.test_loader),
                    data_name=data_name,
                    batch_time=batch_time
            ...
            if self.cfg.data.test.type == "ScanNetPPDataset":
                ...
                pred = pred.cpu().numpy()
                coord = input_dict["coord"].cpu().numpy()
            elif "NerVEDataset" in self.cfg.data.test.type:
                inp = torch.sqrt(torch.sum(pred.mul(pred), dim=-1) + 0.00000001).view(pred.shape[0], 1)
                pred = pred / (inp + 0.0000000001)
                pred = pred.cpu().numpy()
                # pred /= (np.sqrt(np.sum(pred ** 2, axis=-1)) + 1e-8)
        ...
        logger.info(f"Evaluated on {num} examples")
        logger.info("Test result: loss {:.4f}".format(losses / len(self.test_loader)))
        logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
...
class EdgeTesterV4(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            input_dict = data_dict[0]
            data_name = input_dict.pop("name")
            # if data_name != "00000068" and data_name != '00001108':
            #     continue
            pred_save_path = os.path.join(save_path, "{}_pred.xyz".format(data_name))
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
            ...
            if self.cfg.data.test.type == "ScanNetPPDataset":
                ...
                pred = pred.cpu().numpy()
                np.savetxt(pred_save_path, pred)
                # pred /= (np.sqrt(np.sum(pred ** 2, axis=-1)) + 1e-8)
                coord = input_dict["coord"].cpu().numpy()
                np.savetxt(pred_save_path.replace('pred', 'input'), coord)
            else:
                pred = pred.max(1)[1].data.cpu().numpy()
            if "origin_segment" in input_dict.keys():
                assert "inverse" in input_dict.keys()
    ...
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if self.cfg.data.test.type == "ScanNetPPDataset":
                ...
                pred = pred[pred_label == 1].cpu().numpy()
                np.savetxt(pred_save_path, pred)
                # pred /= (np.sqrt(np.sum(pred ** 2, axis=-1)) + 1e-8)
                coord = input_dict["coord"].cpu().numpy()
                np.savetxt(pred_save_path.replace('pred', 'input'), coord)
