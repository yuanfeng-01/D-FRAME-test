...
class MseEvaluator(HookBase):
    ...
    def eval(self):
        ...
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        self.trainer.comm_info["current_metric_value"] = loss_avg*(-1.0)  # save for saver
        self.trainer.comm_info["current_metric_name"] = "val_loss"  # save for saver

    def after_train(self):
        self.trainer.logger.info(
            "Best {}: {:.6f}".format("val_loss", self.trainer.best_metric_value)
        )
...
@HOOKS.register_module()
class EdgeEvaluatorV2(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            loss_pred = output_dict["loss_pred"]

            # Here there is no need to sync since sync happened in dist.all_reduce
            self.trainer.storage.put_scalar("val_loss", loss.item())
            self.trainer.storage.put_scalar("val_seg_loss", loss_seg.item())
            self.trainer.storage.put_scalar("val_pred_loss", loss_pred.item())
            info = "Test: [{iter}/{max_iter}] ".format(
                iter=i + 1, max_iter=len(self.trainer.val_loader)
            )
            if "origin_coord" in input_dict.keys():
            ...
            self.trainer.logger.info(
                info
                + "Loss: {loss:.4f}, Seg loss: {seg_loss:.4f}, Pred loss: {pred_loss:.4f}".format(
                    iter=i + 1, max_iter=len(self.trainer.val_loader), loss=loss.item(), seg_loss=loss_seg.item(), pred_loss=loss_pred.item()
                )
            )
        loss_avg = self.trainer.storage.history("val_loss").avg
        seg_loss_avg = self.trainer.storage.history("val_seg_loss").avg
        pred_loss_avg = self.trainer.storage.history("val_pred_loss").avg
        current_epoch = self.trainer.epoch + 1
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/seg_loss", seg_loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/pred_loss", pred_loss_avg, current_epoch)
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        self.trainer.comm_info["current_metric_value"] = loss_avg*(-1.0)  # save for saver
        self.trainer.comm_info["current_metric_name"] = "val_loss"  # save for saver
...
@HOOKS.register_module()
class EdgeEvaluatorV3(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            loss_seg = output_dict["loss_seg"]
            loss_pred = output_dict["loss_pred"]
            loss_smooth = output_dict["loss_smooth"]

            # Here there is no need to sync since sync happened in dist.all_reduce
            self.trainer.storage.put_scalar("val_loss", loss.item())
            self.trainer.storage.put_scalar("val_seg_loss", loss_seg.item())
            self.trainer.storage.put_scalar("val_pred_loss", loss_pred.item())
            self.trainer.storage.put_scalar("val_smooth_loss", loss_smooth.item())
            info = "Test: [{iter}/{max_iter}] ".format(
                iter=i + 1, max_iter=len(self.trainer.val_loader)
            )
            if "origin_coord" in input_dict.keys():
            ...
            self.trainer.logger.info(
                info
                + "Loss: {loss:.4f}, Seg loss: {seg_loss:.4f}, Pred loss: {pred_loss:.4f}, Smooth loss: {smooth_loss:.4f}".format(
                    iter=i + 1, max_iter=len(self.trainer.val_loader), loss=loss.item(),
                    seg_loss=loss_seg.item(), pred_loss=loss_pred.item(), smooth_loss=loss_smooth.item()
                )
            )
        loss_avg = self.trainer.storage.history("val_loss").avg
        seg_loss_avg = self.trainer.storage.history("val_seg_loss").avg
        pred_loss_avg = self.trainer.storage.history("val_pred_loss").avg
        smooth_loss_avg = self.trainer.storage.history("val_smooth_loss").avg
        current_epoch = self.trainer.epoch + 1
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/seg_loss", seg_loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/pred_loss", pred_loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/smooth_loss", smooth_loss_avg, current_epoch)
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        self.trainer.comm_info["current_metric_value"] = loss_avg*(-1.0)  # save for saver
        self.trainer.comm_info["current_metric_name"] = "val_loss"  # save for saver
...
@HOOKS.register_module()
class EdgeEvaluatorV4(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            self.trainer.logger.info(
                info
                + "Loss: {loss:.4f}, Pred loss: {pred_loss:.4f}, Smooth loss: {smooth_loss:.4f}".format(
                    iter=i + 1, max_iter=len(self.trainer.val_loader), loss=loss.item(),
                    pred_loss=loss_pred.item(), smooth_loss=loss_smooth.item()
                )
            )
        loss_avg = self.trainer.storage.history("val_loss").avg
        pred_loss_avg = self.trainer.storage.history("val_pred_loss").avg
...
@HOOKS.register_module()
class EdgeEvaluatorV5(HookBase):
    def after_epoch(self):
        if self.trainer.cfg.evaluate:
            self.eval()
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            loss = output_dict["loss"]
            loss_seg = output_dict["loss_seg"]
            loss_cd = output_dict["loss_cd"]

            # Here there is no need to sync since sync happened in dist.all_reduce
            self.trainer.storage.put_scalar("val_loss", loss.item())
            self.trainer.storage.put_scalar("val_seg_loss", loss_seg.item())
            self.trainer.storage.put_scalar("val_cd_loss", loss_cd.item())
            info = "Test: [{iter}/{max_iter}] ".format(
                iter=i + 1, max_iter=len(self.trainer.val_loader)
            )
            if "origin_coord" in input_dict.keys():
            ...
            self.trainer.logger.info(
                info
                + "Loss: {loss:.4f}, Seg loss: {pred_loss:.4f}, CD loss: {smooth_loss:.4f}".format(
                    iter=i + 1, max_iter=len(self.trainer.val_loader), loss=loss.item(),
                    pred_loss=loss_seg.item(), smooth_loss=loss_cd.item()
                )
            )
        loss_avg = self.trainer.storage.history("val_loss").avg
        seg_loss_avg = self.trainer.storage.history("val_seg_loss").avg
        cd_loss_avg = self.trainer.storage.history("val_cd_loss").avg
        current_epoch = self.trainer.epoch + 1
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/seg_loss", seg_loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/cd_loss", cd_loss_avg, current_epoch)
        self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
        self.trainer.comm_info["current_metric_value"] = loss_avg*(-1.0)  # save for saver
        self.trainer.comm_info["current_metric_name"] = "val_loss"  # save for saver
